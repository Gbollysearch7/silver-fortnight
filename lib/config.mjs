...
// Railway support: if GOOGLE_SERVICE_ACCOUNT_JSON env var is set,
// write it to a file so scripts can load it from a path
if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON && !process.env.GOOGLE_SERVICE_ACCOUNT_PATH) {
  const saPath = resolve(ROOT_DIR, 'data', 'google-service-account.json');
  if (!existsSync(saPath)) {
    writeFileSync(saPath, process.env.GOOGLE_SERVICE_ACCOUNT_JSON, 'utf-8');
  }
  process.env.GOOGLE_SERVICE_ACCOUNT_PATH = saPath;
}
...
export const SERANKING_API_KEY = process.env.SERANKING_API_KEY;
