...
export function buildFieldData(frontmatter, htmlBody, thumbnailUrl = null, { excludeSlug = false, isFeaturePost = false } = {}) {
  ...
  // Add thumbnail if provided (actual field name is 'main-image' in Webflow)
  if (thumbnailUrl) {
    fieldData['main-image'] = thumbnailUrl;
  }
...
export async function setFeaturedPost(newItemId) {
  // Find the current featured post
  const { items } = await listItems({ limit: 100 });
  const currentFeatured = items?.filter(item =>
    item.fieldData?.['featured'] === true && item.id !== newItemId
  ) || [];

  // Un-feature old ones
  for (const item of currentFeatured) {
    await updateItem(item.id, { 'featured': false });
  }

  // Feature the new one
  await updateItem(newItemId, { 'featured': true });
