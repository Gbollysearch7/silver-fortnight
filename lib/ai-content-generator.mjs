...
If unsure about any claim, write around it using verified facts. NEVER guess.`;
  ...
  // --- Tier 0 & 10: Lean, fast articles (SEO Avalanche foundation) ---
  if (tier <= 10) {
    let prompt = `Write a concise, helpful blog post for TradersYard (a prop trading firm offering funded accounts up to $500K, 80-95% profit split, 1-Step evaluation).

Title: "${title}"
Keyword: "${keyword}"
Target: EXACTLY ${wordCount} words (write the full ${wordCount} words — do not stop short)

Requirements:
- Start with H1 title, then a 2-3 sentence intro paragraph containing the keyword
- IMPORTANT: If the title contains a number (e.g. "15 mistakes"), adjust it to match how many sections you actually write. Never promise more items than you deliver.
- Use 4-6 H2 sections covering the topic directly — no filler
- Include the keyword naturally 3-5 times total
- Add 1 internal link to https://tradersyard.com/#pricing
- Add 1 external authority link to Investopedia (REQUIRED — link to a relevant Investopedia article, e.g. https://www.investopedia.com/terms/r/riskrewardratio.asp or similar). Use a real, relevant Investopedia URL.
${interlinkInstructions}
- End with a "## Frequently Asked Questions" H2 heading, then 2-3 FAQ items using bold "Q:" format
- Close with: [Start your TradersYard challenge today](https://tradersyard.com/#pricing)
- Tone: professional, direct, educational. No fluff.

READABILITY RULES (CRITICAL — follow these exactly):
- Write in SHORT paragraphs: 2-3 sentences max per paragraph. Never write a wall of text.
- Add a blank line between every paragraph for breathing room.
- Vary sentence length — mix short punchy sentences (5-8 words) with medium ones (12-18 words). Never write 3+ long sentences in a row.
- Use bold for key terms and important phrases (2-3 bold items per section).
- Each H2 section should have 2-3 short paragraphs, not one long block.
- Use occasional one-sentence paragraphs for emphasis.
- Break up dense information with a short bullet list (3-4 items max) when listing features, steps, or requirements — but most content should be paragraphs.

${factualAccuracyRules}

Write the complete markdown article now. No frontmatter — start with the H1.`;
  ...
  // --- Tier 20+: Full-length articles with complete SEO optimization ---
...
# TradersYard Context (VERIFIED FACTS ONLY — do not embellish)
TradersYard is an Austrian-based, EU-compliant proprietary trading firm. ONLY state these verified facts:
- One-step evaluation model
- Accounts from £31 up to $500,000
- 80-95% profit split
- Static drawdown (no trailing drawdown)
- Payouts within 24-48 hours
- News trading, EA trading allowed
- Platforms: MT4, MT5, cTrader
- Payment: crypto, bank transfer
...
## On-Page SEO Fundamentals (CRITICAL — follow these exactly)
1. **Primary keyword "${keyword}" placement:**
   - Must appear in the H1 title
   - Must appear in the first 100 words (opening paragraph)
   - Must appear in at least 2 H2 headings naturally
   - Must appear in the closing section
   - Keyword density: 0.8%-1.5% (natural, never forced)
2. **LSI & semantic keywords:** Use variations, synonyms, and related terms throughout. Don't repeat the exact keyword unnaturally.
3. **External authority links (REQUIRED — include at least 2):**
   - **Investopedia (ALWAYS REQUIRED)** — link to a relevant Investopedia article (e.g. https://www.investopedia.com/terms/r/riskrewardratio.asp). Use a real, relevant Investopedia URL for definitions or educational context.
   - Second link: TradingView, BabyPips, DailyFX, CFTC/SEC, Forbes, Bloomberg, or Reuters
   - Use descriptive anchor text, link naturally in context
4. **Internal linking (CRITICAL for SEO):** Include links to TradersYard pages:
   - Link to https://tradersyard.com/#pricing when mentioning challenges or getting started
   - Link to https://tradersyard.com when mentioning the company
${interlinks.length > 0 ? `   - **REQUIRED — Link to these related blog posts naturally within the article:**
${interlinks.map(l => `     - [${l.title}](${l.url})`).join('\n')}
   - Weave these links into relevant paragraphs using descriptive anchor text. Do NOT list them — embed them in sentences.` : '   - Reference related blog topics naturally'}
5. **Heading hierarchy:** H1 → H2 → H3 only. Never skip levels. Each H2 section should cover a distinct subtopic.
6. **URL-friendly slug structure:** Keep section IDs clean and descriptive.

## Writing Style Requirements (VERY IMPORTANT)
- **Write primarily in flowing paragraphs.** This is a well-researched article, NOT a list.
- Each paragraph should be 3-5 sentences. Explain concepts, provide context, tell stories.
- **At least 70% of the article must be paragraph prose.** Maximum 30% can be bullet points or lists.
- Only use bullet points for actual lists (features, steps, requirements, quick comparisons).
- NEVER use a bullet point where a paragraph sentence would work better.
- Vary sentence length — mix short punchy sentences with longer explanatory ones.
- Each H2 section should open with 2-3 paragraphs of prose BEFORE any list or table.
- Transitions between sections should feel natural and connected.

## Structure Requirements
- Start with a compelling hook paragraph (NOT bullets — a real paragraph that draws the reader in)
- Use H2 and H3 headings for scanability
- Each major section needs substantial paragraph content, not just bullet lists
- Include a comparison table where relevant (readers love scannable data)
- End with a **strong closing section** that includes a linked CTA in this exact format:
  [Start your TradersYard challenge today](https://tradersyard.com/#pricing)

## FAQ Section (REQUIRED)
Write a "Frequently Asked Questions" section near the end. Format it EXACTLY like this:

**Q: [Question text here]?**

[Answer in 2-3 sentences as a paragraph.]

**Q: [Another question]?**

[Answer paragraph.]

Include 4-6 FAQ items. Each question must be bolded with "Q:" prefix. Each answer must be a paragraph (not bullets).
...
${factualAccuracyRules}`;
  ...
  // Add research data with actionable instructions
  if (researchData) {
    prompt += buildResearchInstructions(researchData);
  }
...
REMEMBER:
- Write like a journalist, not a list-maker. Paragraphs are your primary tool.
- Every section should have real prose that explains and educates, not just bullet points.
- The FAQ section MUST use the bold "Q:" format specified above.
- The article MUST end with a linked CTA: [Start your TradersYard challenge today](https://tradersyard.com/#pricing)
