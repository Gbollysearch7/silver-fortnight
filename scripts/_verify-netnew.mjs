...
const pub=JSON.parse(readFileSync('data/netnew-published.json','utf8'));
const slugs=Object.values(pub).map(p=>p.slug);
console.log('=== LIVE VERIFICATION ===');
let live=0;
for(const slug of slugs){const code=(await fetch('https://tradersyard.com/blog-posts/'+slug)).status;if(code===200)live++;console.log(`  ${code===200?'✅':'❌'} ${code}  ${slug}`);}
console.log(`${live}/${slugs.length} live\n`);
...
let idx=0;for(const slug of slugs){const r=await fetch('https://indexing.googleapis.com/v3/urlNotifications:publish',{method:'POST',headers:{Authorization:`Bearer ${tok.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({url:'https://tradersyard.com/blog-posts/'+slug,type:'URL_UPDATED'})});if(r.status===200)idx++;await new Promise(r=>setTimeout(r,150));}
